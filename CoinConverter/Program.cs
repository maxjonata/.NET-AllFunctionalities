  ...
  class CoinConverterCmd {
    ...
    public float convertFromCoinAbbreviation(string coinAbbreviationLocal) {
      if(this.quantity == 0 || this.coinAbbreviation == "") {
        ...
        float localCoinValue = APICalls.GetJSONFromRapidAPIURL($"https://exchangerate-api.p.rapidapi.com/rapid/latest/{coinAbbreviationLocal}").GetAwaiter().GetResult().ReadFromJsonAsync<CoinConverterCmd.ExchangeRateAPI_JSON>().GetAwaiter().GetResult().rates[this.coinAbbreviation];
    ...
    public static void Main(string[] args)
    {
      ...
      Console.WriteLine(APICalls.GetJSONFromRapidAPIURL("https://exchangerate-api.p.rapidapi.com/rapid/latest/USD").GetAwaiter().GetResult().ReadFromJsonAsync<CoinConverterCmd.ExchangeRateAPI_JSON>().GetAwaiter().GetResult().time_last_update_utc);
